...
func (d *DirectoryBuilder) Build(directory string) error {
	log := logrus.WithField("dir", directory)

	log.Info("building a manifest specification")
	if err := filepath.Walk(directory, d.LoadBundleWalkFunc); err != nil {
		return err
	}
...
func (d *DirectoryBuilder) LoadBundleWalkFunc(path string, f os.FileInfo, err error) error {
	if f == nil {
